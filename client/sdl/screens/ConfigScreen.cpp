...
// ClientConfig — persistencia en ~/.config/argentum/client.toml
...
ClientConfig ClientConfig::load() {
    ...
    while (std::getline(f, line)) {
        ...
        try {
            if (key == "music_volume")  cfg.musicVolume  = std::stoi(val);
            if (key == "sfx_volume")    cfg.sfxVolume    = std::stoi(val);
            if (key == "fullscreen")    cfg.fullscreen   = (val == "true");
            if (key == "res_width")     cfg.resWidth     = std::stoi(val);
            if (key == "res_height")    cfg.resHeight    = std::stoi(val);
        } catch (...) {}
    }
    return cfg;
}
...
void ClientConfig::save() const {
    ...
    f << "res_width    = " << resWidth     << "\n";
    f << "res_height   = " << resHeight    << "\n";
}
      ...
      config(config), original(config)
{
    ...
    for (int i = 0; i < RES_COUNT; ++i) {
        if (RESOLUTIONS[i].w == config.resWidth &&
            RESOLUTIONS[i].h == config.resHeight) {
            selectedRes = i;
            break;
        }
    }
    ...
    if (e.type == SDL_KEYDOWN) {
        switch (e.key.keysym.sym) {
            ...
            case SDLK_LEFT: case SDLK_a:
                ...
                else if (selectedRow == ROW_RES)
                    selectedRes = (selectedRes - 1 + RES_COUNT) % RES_COUNT;
                applyConfig();
                break;
            ...
            case SDLK_RIGHT: case SDLK_d:
                if (selectedRow == ROW_MUSIC)
                    ...
                    selectedRes = (selectedRes + 1) % RES_COUNT;
                applyConfig();
                break;
            ...
            case SDLK_RETURN: case SDLK_KP_ENTER: case SDLK_SPACE:
                if (selectedRow == ROW_FULLSCR) {
                    ...
                    config.resWidth  = RESOLUTIONS[selectedRes].w;
                    config.resHeight = RESOLUTIONS[selectedRes].h;
                    config.save();
                    applyConfig();
                    out = ScreenResult::GO_MAIN_MENU;
                    return true;
        ...
        for (int row = 0; row < ROW_COUNT; ++row) {
            ...
            if (row == ROW_MUSIC || row == ROW_SFX) {
                if (mx >= sliderX && mx <= sliderX + sliderW) {
                    ...
                    if (row == ROW_MUSIC) config.musicVolume = val;
                    else                  config.sfxVolume   = val;
                    applyConfig();
                }
    ...
    if (e.type == SDL_MOUSEMOTION && (e.motion.state & SDL_BUTTON_LMASK)) {
        ...
        if (selectedRow == ROW_MUSIC || selectedRow == ROW_SFX) {
            if (mx >= sliderX && mx <= sliderX + sliderW) {
                int val = static_cast<int>((float)(mx - sliderX) / sliderW * 100);
                val = std::clamp(val, 0, 100);
                if (selectedRow == ROW_MUSIC) config.musicVolume = val;
                else                          config.sfxVolume   = val;
                applyConfig();
            }
        }
...
void ConfigScreen::renderHeader() {
    ...
    if (t) {
        ...
        tr.y = panel.y + 18;
        drawTex(t, tr.x, tr.y); SDL_DestroyTexture(t);
    }
    SDL_Rect sr{};
    ...
    if (s) {
        ...
        sr.y = panel.y + 55;
        drawTex(s, sr.x, sr.y); SDL_DestroyTexture(s);
    }
}
...
void ConfigScreen::renderOptions() {
    ...
    auto rowLabel = [&](int row, const char* label) {
        ...
        if (lt) {
            ...
            drawTex(lt, lr.x, lr.y); SDL_DestroyTexture(lt);
        }
        return ry;
    };
    ...
    // Volumen musica 
    {
        ...
        if (vt) { vr.x = sliderX + sliderW + 8; vr.y = ry + (ROW_H - vr.h) / 2;
                  drawTex(vt, vr.x, vr.y); SDL_DestroyTexture(vt); }
    }
    ...
    {
        int ry = rowLabel(ROW_RES, "Resolución");
        int rx = sliderX;
        for (int i = 0; i < RES_COUNT; ++i) {
            bool active  = (i == selectedRes);
            std::string lbl = std::to_string(RESOLUTIONS[i].w) + "×" +
                              std::to_string(RESOLUTIONS[i].h);
            SDL_Rect lr{};
            SDL_Texture* lt = makeText(lbl, fontSmall,
                                        active ? C_SEL : C_DIM, lr);
            int boxW = lt ? lr.w + 20 : 80;
            SDL_Rect box = {rx, ry + (ROW_H - 28) / 2, boxW, 28};
            SDL_Color boxBg  = active ? C_SEL_BG : SDL_Color{25, 20, 55, 180};
            SDL_Color boxBrd = active ? C_SEL    : C_PANEL_BORD;
            drawFilledRoundRect(box, boxBg, 4);
            drawBorderRoundRect(box, boxBrd, 4);
            if (lt) {
                lr.x = box.x + (box.w - lr.w) / 2;
                lr.y = box.y + (box.h - lr.h) / 2;
                drawTex(lt, lr.x, lr.y); SDL_DestroyTexture(lt);
            }
            rx += boxW + 8;
        }
        SDL_Rect hr{};
        SDL_Texture* ht = makeText("(se aplica al guardar)", fontSmall, C_DIM, hr);
        if (ht) {
            int ry2 = optStartY + ROW_RES * (ROW_H + ROW_GAP);
            hr.x = sliderX; hr.y = ry2 + ROW_H - hr.h - 4;
            drawTex(ht, hr.x, hr.y); SDL_DestroyTexture(ht);
        }
    }
}
...
void ConfigScreen::renderFooter() {
    ...
    SDL_Color saveBg = hoveredSave
        ? SDL_Color{50, 130, 70, 210} : C_BTN_SAVE;
    ...
    drawBorderRoundRect(bSave, hoveredSave ? C_HOV : C_ON, 5);
    {
        ...
        if (t) { tr.x = bSave.x + (bSave.w - tr.w)/2;
    ...
    SDL_Color cancBg = hoveredCancel
        ? SDL_Color{130, 45, 45, 210} : C_BTN_CANC;
    ...
    drawBorderRoundRect(bCancel, hoveredCancel ? C_HOV : C_OFF, 5);
    {
        ...
        if (t) { tr.x = bCancel.x + (bCancel.w - tr.w)/2;
