...
void EquipmentComponent::applyHelmetToSprite() {
  ...
  sprite.setHelmetTexture(helmetItem.visualTextureId, helmetItem.visualOffsetX,
                          helmetItem.visualOffsetY, helmetItem.iconSrcW,
                          helmetItem.iconSrcH, helmetItem.visualDownSrcX,
                          helmetItem.visualDownSrcY, helmetItem.visualLeftSrcX,
                          helmetItem.visualLeftSrcY, helmetItem.visualRightSrcX,
                          helmetItem.visualRightSrcY, helmetItem.visualUpSrcX,
...
void EquipmentComponent::drawEquipmentLayer(RenderContext &context, const ItemView &item) {
  ...
  SDL_Rect itemSrc{playerSrc.x - sprite.getStartX(),
                   playerSrc.y - sprite.getStartY(), playerSrc.w, playerSrc.h};

  const SpriteSheetConfig cfg = assets.bodyConfigForRace(race);
  SDL_Point offset = visualOffsetForRace(item);

  SDL_Rect itemDest{playerDest.x + offset.x, playerDest.y + offset.y,
                    playerSrc.w * cfg.scale, playerSrc.h * cfg.scale};

  SDL_RenderCopyEx(context.renderer, texture, &itemSrc, &itemDest, 0, nullptr,
                   sprite.spriteFlip);
}
