...
void LobbyClientDeserializersModule::registerDeserializers(Registry &registry) const
{
    ...
    registry.registerDeserializer(
        ...
        [](PacketReader &reader) -> std::unique_ptr<Message>
        {
            ...
            return std::make_unique<CreateGameMessage>(std::move(gameName), maxPlayers);
        });
