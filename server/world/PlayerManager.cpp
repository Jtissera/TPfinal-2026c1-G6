...
void PlayerManager::addPlayer(Player player) {
  ...
  const std::optional<std::pair<int, int>> freeTile =
      findAndOccupyAdjacentTile(tx, ty, id);
  if (!freeTile)
...
int PlayerManager::getTileX(uint32_t id) const {
  return players.at(id).getTileX();
}
int PlayerManager::getTileY(uint32_t id) const {
  return players.at(id).getTileY();
}
int PlayerManager::getPixelX(uint32_t id) const {
  return static_cast<int>(players.at(id).getPixelX());
}
int PlayerManager::getPixelY(uint32_t id) const {
  return static_cast<int>(players.at(id).getPixelY());
}

void PlayerManager::giveExperience(uint32_t playerId, uint32_t exp,
                                   float xpMultiplier) {
  ...
  int16_t newMaxHp =
      formulas.calcMaxHp(p.getRace(), p.getCls(), p.getLevel() + 1);
  int16_t newMaxMana =
      formulas.calcMaxMana(p.getRace(), p.getCls(), p.getLevel() + 1);
...
void PlayerManager::resurrectPlayer(uint32_t id, int spawnTileX,
                                    int spawnTileY) {
...
void PlayerManager::handleResurrectionComplete(uint32_t playerId, int tileX,
                                               int tileY,
...
void PlayerManager::tickPlayerStats(float deltaSeconds,
                                    WorldTickResult &result) {
  for (std::pair<const uint32_t, Player> &entry : players) {
    ...
    const float manaGained =
        player.isMeditating()
            ? formulas.calcManaRegenMeditating(player.getCls(),
                                               player.getRace(), deltaSeconds)
...
void PlayerManager::checkPlayerTileEvents(const MapData &mapData,
                                          WorldTickResult &result) {
  for (std::pair<const uint32_t, Player> &entry : players) {
    ...
    const Tile &tile = mapData.at(static_cast<uint16_t>(player.getTileX()),
    ...
    if (tile.type == TileType::DUNGEON_ENTRANCE ||
      ...
      if (!tile.targetMap.empty())
        result.instanceTransitions.push_back({entry.first, tile.targetMap,
                                              player.getTileX(),
                                              player.getTileY()});
...
PlayerManager::findPlayerIdByName(const std::string &name) const {
...
int PlayerManager::countClanAlliesNear(const Player &player,
...
std::vector<uint32_t>
PlayerManager::getOnlineClanMemberIds(const std::string &clanName) const {
