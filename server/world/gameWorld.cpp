...
Player& GameWorld::getPlayer(uint32_t id)
{
    ...
    if (it == players.end())
        throw std::runtime_error(
            "Player not found: " + std::to_string(id));
...
void GameWorld::giveExperience(uint32_t playerId,
                               uint32_t exp,
                               float xpMultiplier)
{
    ...
    uint32_t limit =
        formulas.calcExpLimit(p.getLevel());
    ...
    int16_t newMaxMana =
        formulas.calcMaxMana(
            p.getRace(),
            p.getCls(),
            p.getLevel() + 1);

    uint32_t finalExp =
        static_cast<uint32_t>(exp * xpMultiplier);
...
GameWorld::DeathResult
GameWorld::handlePlayerDeath(uint32_t targetId,
                             uint32_t attackerId)
{
    ...
    uint32_t safeGold =
        formulas.calcMaxGold(target.getLevel());

    uint32_t excessGold =
        target.die(safeGold);

    std::vector<Item> items =
        target.purgeInventoryOnDeath();
...
void GameWorld::addItemOnGround(Item item,
                                int tileX,
                                int tileY)
{
...
std::optional<Item>
GameWorld::pickItemAt(int tileX, int tileY)
{
    return groundManager.pickItemAt(tileX, tileY);
}

void GameWorld::addGoldOnGround(uint32_t amount,
                                int tileX,
                                int tileY)
{
    groundManager.addGold(
        amount,
        tileX,
        tileY);
}
...
GameWorld::pickGoldAt(int tileX, int tileY)
{
    return groundManager.pickGoldAt(tileX, tileY);
}

void GameWorld::spawnNpc(const std::string& typeName,
                         int tileX,
                         int tileY)
{
...
const std::unordered_map<uint32_t, Npc>&
GameWorld::getNpcs() const
{
    return npcManager.getNpcs();
}

GameWorld::WorldTickResult
GameWorld::tick(float deltaSeconds)
{
...
void GameWorld::tickPlayers(float deltaSeconds,
                            WorldTickResult& result)
{
...
void GameWorld::resurrectPlayer(uint32_t id,
                                int spawnTileX,
                                int spawnTileY)
{
...
std::pair<int, int>
GameWorld::findSafeSpawnNear(int tileX,
                             int tileY) const
{
    for (int dx = -1; dx <= 1; dx++)
    {
...
CityResult GameWorld::handleCityInteraction(
    uint32_t playerId,
    NpcType npcType,
    const CityCommand& cmd)
{
    ...
    return cityDispatcher.dispatch(
        npcType,
        cmd,
        player);
}
...
CityResult
GameWorld::handleRemoteResurrect(uint32_t playerId)
{
    Player& player =
        getPlayer(playerId);
...
std::optional<NpcType>
GameWorld::getNpcTypeAtTile(int tileX,
                            int tileY) const
{
    ...
    return t != NpcType::NONE
               ? std::optional<NpcType>(t)
               : std::nullopt;
}