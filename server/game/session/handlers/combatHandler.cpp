...
void CombatHandler::handleAttackPlayer(uint32_t attackerId, uint32_t targetId,
  ...
  if (attackerTile.zone == ZoneType::SAFE ||
    ...
    sendCombatChat(attackerId, "No podés atacar jugadores en zona segura.",
  ...
  if (!result.valid) {
    if (result.failReason == CombatSystem::Result::FailReason::FRIENDLY_FIRE)
      ...
      sendCombatChat(attackerId,
             ...
             CombatSystem::Result::FailReason::LEVEL_TOO_LOW)
      sendCombatChat(attackerId, "No podés atacar a jugadores de nivel bajo.",
                     ChatMsgType::INFO, monitor);
    else if (result.failReason ==
             CombatSystem::Result::FailReason::LEVEL_DIFF_TOO_HIGH)
      sendCombatChat(attackerId,
  ...
  if (result.dodged) {
    sendCombatChat(attackerId, "¡Tu ataque fue esquivado!", ChatMsgType::INFO,
                   monitor);
    sendCombatChat(targetId, "¡Esquivaste el ataque!", ChatMsgType::INFO,
                   monitor);
    sendStats(attackerId, attacker, monitor);
    sendStats(targetId, target, monitor);
    return;
  ...
  if (result.killed || target.getHp() <= 0) {
    DeathResult deathResult = world.handlePlayerDeath(targetId, attackerId);

    if (deathResult.excessGold > 0)
      monitor.broadcast(std::make_shared<const GoldOnGroundMessage>(
          deathResult.goldInstanceId, deathResult.excessGold, deathResult.tileX,
          deathResult.tileY));
    ...
    sendCombatChat(attackerId, "¡Mataste a " + target.getName() + "!",
    ...
    sendStats(targetId, target, monitor);
    monitor.broadcast(std::make_shared<const PlayerHealthMessage>(
        targetId, static_cast<uint16_t>(target.getHp()),
