...
GameRoom::GameRoom(
    ...
    : gameId(gameId),
      ...
      world(this->mapPath, npcFactory, itemRepo, config),
      gameLoop(
          gameQueue,
          monitor,
          world,
...
uint8_t GameRoom::getPlayerCount() const
{
    return monitor.size();
}

uint8_t GameRoom::getMaxPlayers() const
{
    return maxPlayers;
}
...
{
    return monitor.size() >= maxPlayers;
}
