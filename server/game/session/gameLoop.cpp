...
GameLoop::GameLoop(Queue<ClientMessage> &q, Monitor &m, GameWorld &w,
    ...
    : gameQueue(q), monitor(m), world(w), leaveQueue(leaveQ), transitionQueue(transitionQ),
      gameId(gameId), dispatcher(config, clanManager), statManager(config), tickRateMs(config["server"]["tick_rate_ms"].value_or(33)),
      archive(archive), mapId(mapId), persistEveryNTicks(
...
std::string GameLoop::resolveMapPath(const std::string &targetMap) const
{
  ...
  const std::string pathInMazmorra =
      baseDir + "mazmorra/" + targetMap + ext;
  const std::string pathInCaverna =
      baseDir + "caverna/" + targetMap + ext;
  const std::string pathInRoot =
      baseDir + targetMap + ext;
...
{
  GameWorld::WorldTickResult result = world.tick(deltaSeconds);

  for (const GameWorld::WorldTickResult::ResurrectStartedInfo &info :
       result.resurrectionStarted)
  {
    monitor.sendTo(info.playerId,
                   std::make_shared<ResurrectionStartedMessage>(info.delayMs));
  }

  for (const GameWorld::WorldTickResult::PlayerHit &hit : result.playerHits)
  {
  ...
  for (const GameWorld::WorldTickResult::ClanAllyHit &allyHit :
       result.clanAllyHits)
  {
    std::vector<uint32_t> allies =
        world.getOnlineClanMemberIds(allyHit.clanName);
    const std::string msg =
        "¡Nuestro aliado " + allyHit.targetName + " está siendo atacado!";
  ...
  for (std::pair<uint32_t, GameWorld::DeathResult> &entry :
       result.playerDeathsByNpc)
  {
    const uint32_t playerId = entry.first;
    const GameWorld::DeathResult &deathResult = entry.second;
  ...
  {
    monitor.broadcast(
        std::make_shared<const PlayerDiedMessage>(deadPlayerId));
  }

  for (const GameWorld::WorldTickResult::PlayerResurrection &res :
       result.playersResurrected)
  {
  ...
  for (const GameWorld::WorldTickResult::NpcAttackAnim &atk :
       result.npcAttacksForAnim)
  {
    monitor.broadcast(
  ...
  for (const GameWorld::NpcSpawnEvent &npcSpawn : result.spawnedNpcs)
  {
  ...
  for (GameWorld::InstanceEntry &entry : result.instanceTransitions)
  {
