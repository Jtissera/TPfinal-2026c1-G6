...
int16_t GameFormulas::calcMaxHp(const RaceStats& race,
    ...
    return static_cast<int16_t>(race.constitution * cls.health * race.health * level);
}
...
uint32_t GameFormulas::calcExpOnHit(int16_t damage,
    ...
    int diff = static_cast<int>(targetLevel) - static_cast<int>(attackerLevel) + 10;
    if (diff <= 0) return 0;
...
uint32_t GameFormulas::calcExcessGold(uint32_t gold, uint32_t maxGold) const {
    ...
    if (gold <= maxGold) return 0;
    return gold - maxGold;
}


float GameFormulas::calcHpRegen(const RaceStats& race, float deltaSeconds) const {
    return race.recovery * deltaSeconds;
}

float GameFormulas::calcManaRegen(const RaceStats& race, float deltaSeconds) const {
