...
ClassRepository::ClassRepository(const toml::table& config) {
    ...
    if (!classesSection)
        throw std::runtime_error("ClassRepository: falta la seccion [classes] en el TOML");
...
ClassStats ClassRepository::parse(const std::string& name, const toml::table& entry) const {
    ClassStats stats;
    stats.name        = name;
