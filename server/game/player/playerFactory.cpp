...
Player PlayerFactory::create(uint32_t clientId, const std::string &name,const std::string &raceName,const std::string &className, int spawnTileX,int spawnTileY) const {
    const RaceStats& raceStats = raceRepo.get(raceName);
    const ClassStats& classStats = classRepo.get(className);

    const int16_t maxHp = formulas.calcMaxHp(raceStats, classStats, 1);
    const int16_t maxMana = formulas.calcMaxMana(raceStats, classStats, 1);
