...
public:
    ...
    int16_t calcMaxHp(const RaceStats &race,
                      const ClassStats &cls,
                      uint8_t level) const;

    int16_t calcMaxMana(const RaceStats &race,
                        const ClassStats &cls,
                        uint8_t level) const;
    ...
    uint32_t calcExpOnHit(int16_t damage,
                          uint8_t attackerLevel,
                          uint8_t targetLevel) const;

    uint32_t calcExpOnKill(int16_t targetMaxHp,
                           uint8_t attackerLevel,
                           uint8_t targetLevel) const;
    ...
    float calcManaRegenMeditating(const ClassStats &cls,
                                  const RaceStats &race,
                                  float deltaSeconds) const;

private:
    const toml::table &config;
    ZoneNaming zoneNaming;
};