...
void MapCanvas::setMap(MapData *map)
{
    ...
    if (_map)
    {
        setMinimumSize(_map->width() * tileSize(), _map->height() * tileSize());
    }
    update();
}
...
QColor MapCanvas::tileColor(const Tile &tile) const
{
    ...
    case TileType::CITY_FLOOR:
        base = QColor(80, 60, 40);
        break;
    case TileType::HOUSE:
        base = QColor(120, 80, 50);
        break;
    case TileType::CHURCH:
        base = QColor(100, 100, 140);
        break;
    case TileType::MILL:
        base = QColor(160, 130, 90);
        break;
    case TileType::EXIT:
...
void MapCanvas::drawNpcIndicator(QPainter &painter, const QRect &r,
                                 NpcType npc) const
{
    if (npc == NpcType::NONE)
        return;
...
bool MapCanvas::screenToTile(const QPoint &pos, uint16_t &tx,
                             uint16_t &ty) const
{
    if (!_map)
        return false;
