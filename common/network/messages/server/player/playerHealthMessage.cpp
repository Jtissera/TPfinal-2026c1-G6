...
PlayerHealthMessage::PlayerHealthMessage(uint32_t playerId, uint16_t hp, uint16_t hpMax) :
    playerId(playerId),hp(hp),hpMax(hpMax)
{}
...
uint32_t PlayerHealthMessage::getPlayerId() const {
    return playerId;
}

uint16_t PlayerHealthMessage::getHp() const {
    return hp;
}

uint16_t PlayerHealthMessage::getHpMax() const {
    return hpMax;
}
