...
NpcHealthMessage::NpcHealthMessage(uint32_t npcId, int16_t hp, int16_t maxHp)
    : npcId(npcId),
      hp(hp),
      maxHp(maxHp)
{
...
uint32_t NpcHealthMessage::getNpcId() const { return npcId; }
uint16_t NpcHealthMessage::getHp() const { return hp; }
uint16_t NpcHealthMessage::getMaxHp() const { return maxHp; }
