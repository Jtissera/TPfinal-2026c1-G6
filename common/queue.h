...
public:
    ...
    explicit Queue(const unsigned int max_size): base(max_size) {}

    Queue() = delete;

    void close() { base.close(); }

    bool try_push(T* const& val) { return base.try_push(val); }

    bool try_pop(T*& val) {
        void* tmp = nullptr;
        bool result = base.try_pop(tmp);
        val = static_cast<T*>(tmp);
        return result;
    }

    void push(T* const& val) { base.push(val); }
